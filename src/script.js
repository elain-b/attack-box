...
document.addEventListener('keydown', (event) => {
    ...
    if (event.key == "w") {
        ...
        // boxes_mesh[0].velocity.set(1 ,0, 0)
    }
    if (event.key == "s") {
        // (moveForward)[event.key.toLowerCase()] = true
        boxes_mesh[0].position.x -= 0.05
        // boxes_mesh[0].velocity.set(5 ,0, 0)
    }
    if (event.key == "d") {
        // (moveForward)[event.key.toLowerCase()] = true
        boxes_mesh[0].position.z -= 0.05
        // boxes_mesh[0].velocity.set(5 ,0, 0)
    }
    if (event.key == "a") {
        // (moveForward)[event.key.toLowerCase()] = true
...
new GLTFLoader().load('/models/box2.glb', function (gltf) {
    ...
    const body = new CANNON.Body({
        mass: 1,
        position: new CANNON.Vec3(0, 0.5 * 0.5, 0),
        shape: shape,
        material: defaultMaterial
    })
    body.addShape(shape)
...
const tick = () => {
    ...
    for (const object of objectsToUpdate2) {
        object.model.traverse(function (obj) {
            // obj.position.copy(object.body.position)
            obj.position.x = object.body.position.x * 0.5
            obj.position.y = object.body.position.y * 0.5
            obj.position.z = object.body.position.z * 0.5
            obj.quaternion.copy(object.body.quaternion)
        })
    }
